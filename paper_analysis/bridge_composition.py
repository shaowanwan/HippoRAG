"""Bridge composition: classify LLM-discovered bridges into 5 categories and plot.
Categories (priority): Answer / Hop-intermediate / Query / Contextual(evidence|distractor).
Contextual split = graph entity-passage connectivity to gold (resolver: exact+embedding, th=0.55).
Run from project root: .venv/bin/python paper_analysis/bridge_composition.py
"""
...
INDEX = "outputs/musique_ner_pipeline_eval/global_ner_index.pkl"
RUN = "outputs/musique_ner_pipeline_eval/comparison_results_rounds3_20260421_143341_41732.json"
OUT = "outputs/bridge_composition_4way"
...
def sq(a):
    ...
    a = ''.join(c for c in a.lower() if c not in ex)
...
def main():
    ...
    for r in d:
        ...
        for name in {x for x in raw if x}:
            b = sq(name)
            if b == final or b in final or final in b: C['Answer'] += 1
            elif any(b == h or b in h or h in b for h in inter if h): C['Hop-intermediate'] += 1
    ...
    for i, bar in enumerate(bars):
        ...
        ax.text(bar.get_x() + bar.get_width() / 2, vals[i] + 0.8, f'{vals[i]:.1f}%', ha='center', fontsize=10, fontweight='bold')
    ...
    ax.set_title(f'Composition of LLM-discovered bridge entities (MuSiQue, {tot} bridges / 1000 q)', fontsize=10.5)
